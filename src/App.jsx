...
// Import tvých datových modulů - přesně podle tvého nastavení
import { allData } from './data';
import { prikladyData } from './data/prikladyData';
import { maDatiDalData } from './data/matidal';

// Sjednocení dat: Vše z data.js (Základy, Majetek, Zdroje, Rozvaha, Účtování) 
// + přidáme MD/Dal a Trenažér
const finalData = [
  ...allData,       // Tady už máš 5 kapitol z data.js
  maDatiDalData,    // 6. kapitola
  prikladyData      // 7. kapitola (Trenažér)
];
...
function PracticeSection({ cviceni }) {
  ...
  return (
    <div className="mt-12 p-8 bg-zinc-50 dark:bg-zinc-900/50 rounded-[2.5rem] border-2 border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-200/50 dark:shadow-none transition-all">
...
function TemaLayout({ isDark, setIsDark }) {
  ...
  return (
    <div className="flex min-h-screen bg-white dark:bg-black text-zinc-900 dark:text-zinc-100 transition-colors">
      <aside className="hidden lg:block w-80 border-r border-zinc-200 dark:border-zinc-900 p-10 sticky top-0 h-screen overflow-y-auto bg-zinc-50/50 dark:bg-black">
        <Link to="/" className="text-[10px] font-black text-blue-600 uppercase mb-12 block tracking-[0.2em] hover:translate-x-[-4px] transition-transform">← Zpět na přehled</Link>
        ...
        <nav className="space-y-6">
          {tema.podtemata.map(p => (
            <Link key={p.id} to={`/${tema.id}/${p.id}`} className={`block text-[11px] font-black uppercase tracking-wider transition-all ${podtemaId === p.id ? 'text-blue-600 border-l-2 border-blue-600 pl-4' : 'text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 pl-4'}`}>
              {p.titul}
            </Link>
          ))}
        </nav>
      </aside>
      <main className="flex-1 max-w-4xl mx-auto p-8 md:p-16 lg:p-24">
        <header className="mb-16">
            <span className="text-[10px] font-black text-blue-600 uppercase tracking-[0.4em] block mb-4 italic">Kapitola</span>
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-zinc-900 dark:text-white leading-[0.95]">{podtema.titul}</h1>
        </header>
            ...
            <Link 
              ...
              className={`p-10 rounded-[2.5rem] border-2 transition-all group flex flex-col justify-between min-h-[300px] shadow-sm hover:shadow-2xl hover:-translate-y-2
                ${t.id === 'priklady-uctovani' 
                  ...
                  : 'border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/30 hover:border-blue-500 dark:text-white'}`}
            >
              <div>
                ...
                <h2 className={`text-4xl font-black uppercase tracking-tighter leading-tight ${t.id === 'priklady-uctovani' ? 'text-white' : 'text-zinc-900 dark:text-white group-hover:text-blue-600 transition-colors'}`}>
                  {t.titul}
                </h2>
              </div>
              <div>
                ...
                <p className={`text-[10px] font-black uppercase tracking-widest ${t.id === 'priklady-uctovani' ? 'text-white/60' : 'text-zinc-400'}`}>
                  {t.podtemata.length} lekcí k prostudování
                </p>
              </div>
            </Link>
          ))}
